...
public class FieldTypeTest {
    ...
    File indexPath = new File("fieldType");
    ...
    public void testIndextCreate() throws IOException {
        ...
        fieldType.setIndexed(true);//字段是否需要索引
        fieldType.setStored(true);//字段是否需要存储
        fieldType.setTokenized(true);//字段是否需要使用分词器进行分词

        FieldType fieldType2 = new FieldType();
        fieldType2.setIndexed(true);//字段是否需要索引
        fieldType2.setStored(true);//字段是否需要存储
        fieldType2.setTokenized(false);
        //创建文档对象
        Document document1 = new Document();
        document1.add(new Field("title","content1",fieldType));
        document1.add(new Field("content",content1,fieldType2));
        //将文档写入
        indexWriter.addDocument(document1);

        Document document2 = new Document();
        document2.add(new Field("title","content2",fieldType));
        document2.add(new Field("content",content2,fieldType2));
        indexWriter.addDocument(document2);

        Document document3 = new Document();
        document3.add(new Field("title","content3",fieldType));
        document3.add(new Field("content",content3,fieldType2));
    ...
    public void testSearch() throws IOException, ParseException {
        ...
        Query query = queryParser.parse("content1");
