...
public class CRUDTest {
    ...
    public void testUpdate() throws Exception{
        Directory directory = FSDirectory.open(indexPath);
        IndexWriterConfig indexWriterConfig = new IndexWriterConfig(matchVersion, analyzer);
        //indexWriterConfig.setOpenMode(IndexWriterConfig.OpenMode.CREATE);//索引每次都重新创建
        IndexWriter indexWriter = new IndexWriter(directory, indexWriterConfig);
        FieldType fieldType = new FieldType();
        fieldType.setIndexed(true);
        fieldType.setStored(true);
        Document updateDocument = new Document();
        updateDocument.add(new Field("id","2",fieldType));
        updateDocument.add(new Field("title","content2",fieldType));
        updateDocument.add(new Field("content","hello 这是我修改的内容",fieldType));
        indexWriter.updateDocument(new Term("id","2"),updateDocument);
        indexWriter.commit();
        indexWriter.close();
        testSearch();
    }
